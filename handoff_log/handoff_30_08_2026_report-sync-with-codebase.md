...
## 4. ⚠️ ตอน push เจอว่าเพื่อน push งานใหญ่มาก่อน — ต้อง sync รายงานรอบสอง

`git push` ถูก reject เพราะ remote มี 3 commits ที่เราไม่มี (`013834a test` · `4168041 fix_bullmq_dash` · `285b233 dashbord`) — **เพิ่ม observability layer ทั้งชุด** 30 ไฟล์ / +4,010 บรรทัด

merge สะอาด ไม่มี conflict (งานเราเป็น `docs/` + `handoff_log/` ล้วน ไม่ทับกัน) แต่ของใหม่แตะไฟล์ที่รายงานอ้างถึงหลายตัว จึงต้อง **ตรวจซ้ำว่าที่แก้ไปเมื่อกี้ยังจริงอยู่ไหม**

**ข่าวดี: การเปลี่ยนแปลงของเพื่อนเป็น instrumentation ล้วน** — ไล่ diff ของ `orders.service.ts`, `orders.processor.ts`, `products.service.ts` แล้วมีแต่บรรทัด `this.metrics.inc(...)` แทรกเข้ามา **control flow / นโยบายชดเชย / gatekeeper / การเทียบ requestToken ไม่ถูกแตะเลยแม้แต่บรรทัดเดียว** → §2.4 และ §2.5 ที่เพิ่งแก้ไปยังถูกต้องทั้งหมด

สิ่งที่ต้องแก้เพิ่มเพราะของใหม่:

| จุด | เดิม | แก้เป็น |
| :--- | :--- | :--- |
| §1.3 บรรทัดเทคโนโลยี | "35 unit tests" | **43** (`integrity.service.spec.ts` เพิ่มมา 8 ข้อ · อีก 3 ไฟล์ +4 บรรทัดเป็น mock ของ `MetricsService` ไม่ใช่เทสต์ใหม่) |
| §3 | ไม่มีเรื่อง observability เลย | เพิ่ม **§3.8** — ตาราง 3 หน้า (`/admin/queues`, `/admin/insights`, `/admin/metrics`), วิธีอ่าน `drift` (ติดลบ = ปกติ · เป็นบวก = อันตราย), ตัวนับที่ต้องเฝ้า (`stock_compensation_failures_total`) และเหตุผลที่ metrics เป็น write-behind |
| §4 คอขวดที่เหลือ | "ไม่มี reconciliation Redis ↔ DB" | **กลายเป็นเท็จแล้ว** — มีตัวตรวจแล้ว (ทุก 3 วิ) แต่จงใจไม่มีตัวซ่อมอัตโนมัติ · ข้อจำกัดที่เหลือคือไม่มีใครเฝ้าหน้าจอ |
| เลขตาราง | 8/9/10 | เลื่อนเป็น **9/10/11** เพราะ §3.8 แทรกตารางที่ 8 เข้ามา |

> **บทเรียน**: `git push` ที่ถูก reject ไม่ใช่แค่เรื่อง merge — ถ้างานที่เรากำลัง commit เป็น **เอกสารที่บรรยายโค้ด** ต้องถือว่า commit ที่ merge เข้ามาอาจทำให้เอกสารที่เพิ่งเขียนเสร็จกลายเป็นของเก่าทันที ต้องไล่ diff ของไฟล์ที่เอกสารอ้างถึงทุกครั้ง

## 5. 🚧 ค้างไว้ให้คนถัดไป

| เรื่อง | สถานะ |
| :--- | :--- |
| **ตัวเลขผลทดสอบในรายงานยังเป็นของ 28 ส.ค.** | 2,548 rps · read p95 469.79ms · write p95 317.34ms · hit 97.63% — **ยังไม่ได้ยิงซ้ำ** · ⚠️ ตอนนี้มีเหตุให้ต้องยิงใหม่แล้ว เพราะ observability ที่ merge เข้ามาแทรก `metrics.inc()` ลงเส้นทางร้อนทั้ง read และ write (ออกแบบเป็น write-behind ให้ถูกแล้ว แต่ **ยังไม่มีใครวัดว่ากระทบ throughput จริงเท่าไร**) |
| **ยังไม่ได้รัน build/lint/test หลัง merge** | งานเราเป็น `docs/` ล้วน แต่ commit ที่ merge เข้ามาแตะ `src/` 15+ ไฟล์ · ควรรัน `pnpm run build && pnpm run test` ยืนยัน 43 ข้อสักครั้ง (⚠️ ช้ามากบนเครื่องนี้ — test ~550 วิ) |
| **`redis.keys.ts` / `orders.*.ts` กลายเป็น mode 100755** | มาจาก commit ของเพื่อน ไม่ใช่ SynologyDrive ของเรา — ยังไม่ได้แก้ |
