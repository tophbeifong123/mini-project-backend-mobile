...
> ⚠️ **ขอบเขตของตัวเลขชุดนี้** — วัดบนเครื่อง dev ที่รัน k6 ไปพร้อมกัน **ไม่ใช่ VM เป้าหมาย 4-core/6GB** และวัด**ก่อน**การปรับขนาดทรัพยากรเมื่อ 31 ส.ค. 2569 (ปรับ `cpus` ของ app เป็น `1.0`/instance รวมทั้งสแตก 10.0 · `mem_limit` รวม 5,440 MB = 88.5% ของ VM · `WORKER_CONCURRENCY` 5 → 1) นอกจากนี้การวัดซ้ำบนเครื่องเดียวกันมี noise สูงมาก (ยิง 200 VUs ซ้ำ 7 รอบได้ 700–5,406 req/s ต่างกัน 7.7 เท่า ตามภาระของ host) ตัวเลขจึงใช้เทียบ **แนวโน้มก่อน/หลังปรับจูนของระบบเราเอง** ได้ แต่ยังไม่ควรใช้เทียบข้ามกลุ่มโดยตรงจนกว่าจะยิงบน VM เป้าหมายด้วยคอนฟิกปัจจุบัน
...
> ⚠️ รอบนี้ยิงด้วย**คอนฟิกก่อนการปรับขนาดเมื่อ 31 ส.ค. 2569** (ดูกรอบใต้ตารางที่ 5) — ยังไม่มีใครยิงซ้ำด้วยคอนฟิกปัจจุบัน และ **การยิงข้ามกลุ่มจริงยังไม่เคยเกิดขึ้น** ตารางที่ 9 จึงยังว่างอยู่
...
> **ข้อจำกัดที่ต้องรู้ก่อนใช้ตัวนับนี้ — "เป็นศูนย์" ยังไม่ใช่หลักฐานว่าไม่รั่ว** ตัวนับนี้ถูกยิงจาก **เส้นทาง enqueue เท่านั้น** (`orders.service.ts:269` และ `:288`) ส่วนเส้นทาง worker ไม่เคยยิงมันเลย เพราะที่ `orders.processor.ts:126` โค้ดบวก `stock_compensated_total` **ก่อน** จะ `await compensateOnce(...)` และ `await` ตัวนั้นไม่มี `try/catch` คลุม ผลคือถ้าการชดเชยฝั่ง worker ล้มเหลว มันจะถูกนับเป็น "ชดเชยสำเร็จ" ไปแล้วหนึ่งครั้ง ขณะที่ตัวนับความล้มเหลวยังคงเป็นศูนย์ พูดอีกแบบคือตัวนับนี้ **จำเป็นแต่ไม่เพียงพอ** — ค่าที่ไม่เป็นศูนย์เชื่อได้ว่ารั่วจริง แต่ค่าศูนย์เชื่อไม่ได้ว่าไม่รั่ว ตัวจับความจริงที่ยังใช้ได้ในกรณีนี้คือคอลัมน์ `drift` ใน `/admin/insights` และการตรวจตามหัวข้อ 3.7 หลังจบการทดสอบ
...
สแตกนี้ถูกตั้งค่าไว้สำหรับการทดสอบบนเครื่องตัวเอง ก่อนยกไปวางบนเครือข่ายเดียวกับกลุ่มอื่นจึงตรวจสามจุด — **ปิดไปแล้ว 1 · ยังเปิดอยู่ 1 · ตรวจแล้วว่าแน่นดี 1**

- **✅ พอร์ตของ datastore — ปิดแล้ว (31 ส.ค. 2569)** เดิม PostgreSQL `5432`/`5433` และ Redis ทั้งสองตัวถูก publish ออกมาโดยไม่ผูกกับ interface ใด ประกอบกับ `redis/redis-data.conf:10` ตั้ง `protected-mode no` และไม่มี `requirepass` ใครบนวง LAN เดียวกันจึงเขียน datastore ได้ตรง ๆ · แก้แล้วโดยผูกทั้ง 4 พอร์ตกับ loopback (`docker-compose.yml:52,108,151,177` → `"127.0.0.1:5432:5432"` ฯลฯ) ส่วน `8080:80` (`:596`) **จงใจไม่แตะ** เพราะกลุ่มอื่นต้องยิง k6 เข้ามาได้ · ยืนยันด้วยการยิงจริงจาก LAN IP: PostgreSQL/Redis **BLOCKED ครบทั้ง 4** ขณะที่ nginx ยัง OPEN · เครื่องมือฝั่งโฮสต์ไม่กระทบเลย (`redis-cli -p 6380 GET` ในหัวข้อ 3.7, `pnpm run seed:redis`, `pnpm run reset` ทดสอบแล้วผ่าน เพราะทุกตัวใช้ loopback อยู่แล้ว)
  > 📌 **แก้คำอธิบายเดิมที่เขียนไว้ไม่แม่น** — เดิมเขียนว่าการเขียน Redis ตรง ๆ "ข้ามการป้องกันทั้ง 4 ชั้น" ซึ่ง**ไม่จริง** การเขียน Redis ข้ามได้แค่ **Tier 1** (gatekeeper) ส่วน **Tier 3** (`WHERE remaining_stock > 0`) และ **Tier 4** (`CHECK remaining_stock >= 0`) ยังกันการขายเกินอยู่ อาการจริงคือผู้ใช้ได้ 202 แล้ว job ตายเป็น `SoldOutError` = พังเชิงพฤติกรรม ไม่ใช่ขายเกิน · **ตัวที่ข้ามได้ทุกด่านจริงคือ PostgreSQL** (รหัส `flashsale`/`flashsale` อยู่ในไฟล์ที่ push ขึ้น GitHub สาธารณะ) เพราะ Tier 3/4 อยู่ *ใน* PostgreSQL เอง
- **รหัสผ่าน Bull-Board เป็นค่า `admin`/`admin` ที่ hardcode ไว้** — ทั้ง 6 บริการกำหนดค่านี้เป็นสตริงตรง ๆ ไม่ใช่ `${VAR}` (`docker-compose.yml:245-246` และอีก 5 ชุดที่เหมือนกันที่ `:309-310`, `:372-373`, `:435-436`, `:498-499`, `:561-562`) และ `env.validation.ts:133,137` ก็ตั้ง default เป็น `'admin'` ไว้อีกชั้น `getOrThrow()` จึงไม่มีวัน throw แม้ไม่ได้ตั้งค่าใด ๆ ข้อควรรู้คือการแก้ `.env` **ไม่มีผล** กับ container ต้องแก้ที่ `docker-compose.yml` เท่านั้น
- **ส่วนที่ตรวจแล้วว่าแน่นดี** — การครอบ Basic Auth ที่ prefix `/admin` เพียงจุดเดียวใน `main.ts:52` ถูกทดสอบด้วยรูปแบบ URL ที่ใช้หลบ middleware 11 แบบ (สลับตัวพิมพ์ใหญ่-เล็ก, URL-encoding, สแลชซ้อน, dot-segment) **ไม่มีรูปแบบใดเล็ดลอดผ่าน guard ไปได้** จุดที่ต้องแก้จึงเป็นตัวรหัสผ่านและพอร์ตที่เปิดไว้ ไม่ใช่วิธีการ mount
