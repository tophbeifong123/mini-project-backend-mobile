...
> `build` / `lint` / `test` (59 tests) ผ่านหมด · รันบน container จริงและยิง k6 มาแล้วหลายรอบ (§9.3 ผ่านครบ)
...
| **config ที่ right-size แล้วยังไม่เคยยิงบน VM เป้าหมายจริง** | ✅ **พิสูจน์บน VM 4-core/6GB แล้ว (2026-09-01)** — API 6 containers ใช้ `cpus: 1.0`; ปิด consumer ด้วย `ORDER_WORKER_ENABLED=false` และแยก `orders-worker` (`cpus: 0.25`, concurrency 1) เพื่อไม่ให้ DB transaction แย่ง HTTP event loop · `loadtest.js` 3 รอบล่าสุดได้ median read p95 **291.00ms**, write p95 **1,140.77ms**, 5xx/contract violation = 0 และ §9.3 ผ่านครบ · `mpstat` ระหว่างยิงยืนยัน CPU 95–100%, iowait/steal = 0 จึงเหลือเพดาน CPU จริง ไม่ใช่ datastore/network |
...
| ~~`loadtest.js` threshold มีแค่ latency~~ | ✅ **ปิดไปแล้ว — ตรวจซ้ำ 2026-08-31 พบว่าแก้ไปแล้ว แต่ตารางนี้ยังค้างอยู่** · `loadtest.js:92-107` มี threshold ครบทั้ง 3 ตัวที่เคยขาด (`reads_bad_contract: ['count==0']`, `orders_unexpected_status: ['count==0']`, `orders_unauthorized_401: ['count==0']`) **บวกอีก 5 ตัวที่ไม่เคยระบุไว้เดิม** (`checks: ['rate>0.99']`, `http_req_failed: ['rate<0.01']`, `reads_infra_timeout_or_5xx: ['count==0']`, `reads_remaining_stock_present: ['rate>0.99']`, `orders_accepted_202: ['count==50']`, `orders_not_seeded_503: ['count==0']`) · เลข `:81-84` เดิมก็เพี้ยน — ตอนนี้ช่วงนั้นคือ scenario `write_burst` ไม่ใช่บล็อก threshold · **k6 ล้มเสียงดังตามที่ควรแล้ว** |
...
| [`docs/Architecture/architecture-rationale.md`](docs/Architecture/architecture-rationale.md) | **เหตุผลการออกแบบ + ข้อดีข้อเสีย + บันทึก design review** — อ่านก่อนจะแก้ดีไซน์ (§7: blocker (a)/(b) **ปิดแล้ว** เหลือ "ยืนยันพฤติกรรม BullMQ ด้วยการรันจริง" ที่ยังเปิด เพราะไม่มี e2e test) |
...
| [`docs/Summary_Best_Practice/architecture_BP_Checklist.md`](docs/Summary_Best_Practice/architecture_BP_Checklist.md) | ⭐ **กฎบทเรียน 162 ข้อ → โค้ดจริงบรรทัดไหน** — ตารางเต็ม B01–06 + slide-errata 43 ข้อ + ช่องว่างที่ควรพูดในรายงาน (ตรวจ 2026-08-31) |
